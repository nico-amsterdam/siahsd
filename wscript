...
def build(bld):
    ...
    bld.stlib(source="hook_script.c", target="script", use='glib-2.0')
    bld.stlib(source="config.c", target="config", use='glib-2.0 database jsonbot spacestate script')
    ...
    bld.program(
                source = 'siahsd.c',
                target = 'siahsd',
                use    = [ 'database', 'config', 'status', 'sia', 'siahs', 'jsonbot', 'spacestate', 'script', 'dbi', 'talloc', 'glib-2.0', 'nettle' ])

    bld.program(
                source = 'secip.idl secipd.c crc16.c',
                target = 'secipd',
                use    = [ 'database', 'config', 'status', 'sia', 'siahs', 'jsonbot', 'spacestate', 'script', 'dbi', 'samba', 'glib-2.0', 'nettle', 'ndr' ])
    bld.program(
